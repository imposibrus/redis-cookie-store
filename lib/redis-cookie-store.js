...
RedisCookieStore.prototype.getAllCookies = function getAllCookies(cb) {
  var self = this;
  ...
  this.client.keys(this.prefix + '*', function(err, keys) {
    ...
    async.parallel(keys.map(function(key) {
      return function(callback) {
        var domain = key.replace(self.prefix, '').split(':')[0];

        self.client.hgetall(key, function(err, hash) {
          var cookiesArr = _.values(hash).map(function(cookieString) {
            var cookie = Cookie.parse(cookieString);
          ...
          callback(err, cookiesArr);
        });
      };
    }), function(err, cookiesArrArr) {
      if (err) {
        return cb(err);
      }

      cb(null, _.flatten(cookiesArrArr));
    });
  });
};
